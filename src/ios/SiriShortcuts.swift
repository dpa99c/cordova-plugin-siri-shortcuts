...
@objc(SiriShortcuts) class SiriShortcuts: CDVPlugin, APPAppEventDelegate {
  @available(iOS 12.0, *)
  ...
  @objc(present:) func present(_ command: CDVInvokedUrlCommand) {
    self.commandDelegate!.run(inBackground: {
      if #available(iOS 12.0, *) {
        ...
        self.getVoiceShortcut(
          persistentIdentifier: persistentIdentifier,
          completion: { [unowned self] inVoiceShortcut in
            if let inVS = inVoiceShortcut {

              self.shortcutEditPresentedDelegate = ShortcutEditPresentedDelegate(
                command: command, shortcuts: self)
              ...
              if self.activity != nil {
                self.shortcutPresentedDelegate = ShortcutPresentedDelegate(
                  command: command, shortcuts: self)
    ...
    self.commandDelegate!.run(inBackground: {
      if #available(iOS 12.0, *) {
        ...
        NSUserActivity.deleteSavedUserActivities(
          withPersistentIdentifiers: persistentIdentifiers,
          completionHandler: {
            self.sendStatusOk(command)
          })
      } else {
    ...
    self.commandDelegate!.run(inBackground: {
      if #available(iOS 12.0, *) {
        INVoiceShortcutCenter.shared.getAllVoiceShortcuts {
          [unowned self] (voiceShortcutsFromCenter, error) in
